...
- Persistence: a manually-managed TypeORM `DataSource` singleton (`AppDataSource` in `src/database/database.providers.ts`), not `TypeOrmModule.forRootAsync` — provided under the `DATA_SOURCE` token by a small `DatabaseModule` (`src/database/database.module.ts`) that both `AppModule` and `UsersModule` import (a provider declared directly on `AppModule` isn't visible to sibling/leaf modules without this). It connects lazily on first real use (`ensureInitialized()`), not at boot, so the app starts and serves non-DB routes even when Postgres is unreachable; a request that actually needs the DB gets a clean `503` (via `isConnectivityError()`, which classifies both a connection that was never established and one that dies mid-lifetime) instead of hanging or a raw 500. `synchronize` is hardcoded `false` — there is no migration system yet, so schema changes need manual DDL in every environment, not just production.
- Auth: `passport-jwt` strategy (`modules/auth/jwt.strategy.ts`) backing a guard (`modules/auth/jwt-auth.guard.ts`) that's a bare `AuthGuard('jwt')` subclass with no constructor dependencies — other modules can import the guard class directly without pulling in all of `AuthModule`. `JWT_EXPIRY` is a plain seconds count (e.g. `"3600"`), not a duration string — it's parsed with `Number.parseInt`, never cast. `ConfigService` (`src/config/config.service.ts`) centralizes the environment-gated decisions `AuthService` depends on — `isProduction()`, `isDevLoginBypassEnabled()`, `isRegistrationAllowed()` — rather than each call site doing its own raw `process.env.NODE_ENV` comparison.
...
- Required env vars live in `apps/api/.env` (gitignored): `DATABASE_URL`, `JWT_SECRET`, `JWT_EXPIRY`, `CORS_ORIGIN`, `PORT`, `NODE_ENV`. Optional: `ALLOW_DEV_LOGIN_BYPASS` — must be set to exactly `"true"` to enable `AuthService.login()`'s no-password dev shortcut (`ConfigService.isDevLoginBypassEnabled()`); deliberately *not* tied to `NODE_ENV` alone (a stray `NODE_ENV=test` must never be enough on its own to bypass a password check), and still blocked outright whenever `NODE_ENV=production`, regardless of this flag.
